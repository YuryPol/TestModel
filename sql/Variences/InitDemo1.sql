...
DROP PROCEDURE IF EXISTS ExpandStructData;
DELIMITER //
CREATE PROCEDURE ExpandStructData(IN iset BIGINT, IN amount INT)
BEGIN
-- find sets to be unionized and insert their unions
...
--    itterate_nodes: LOOP
--    END IF
--    SELECT availability_excl INTO cnt FROM structured_data_inc WHERE set_key = iset; 
--    IF cnt >= amount
--    THEN
--     -- decrement availability_excl
--     UPDATE structured_data_inc 
--     SET availability_excl=cnt-amount 
--     WHERE set_key = iset;     
--     LEAVE itterate_nodes;
--    ELSE
--     amount = amount - cnt;
--     UPDATE structured_data_inc 
--     SET availability_excl=0;
--     WHERE set_key = iset;     
--     -- move to the next nodes
--     ...
--    END IF;    
    
DROP FUNCTION IF EXISTS ExtendSD;
DELIMITER //
CREATE FUNCTION ExtendSD(iset BIGINT, amount INT)
RETURNS BOOLEAN
DETERMINISTIC
READS SQL DATA
BEGIN
    ...
    SELECT 
       ...
       ri.basesets AS set_key    
    FROM structured_data_inc sd
    WHERE availability_excl > 0 
       AND availability_excl =< amount
       AND set_key & iset = iset
       AND extended = false
    JOIN raw_inventory ri 
       ON sd.set_key & ri.basesets != 0
       AND BIT_COUNT(sd.set_key)+1 = BIT_COUNT(ri.basesets) -- add nodes of higher rank
